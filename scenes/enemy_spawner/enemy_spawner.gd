...
func get_random_position():
	...
	# 屏幕外这个范围内随机生成
	var base_x = randi_range(30, 100)
	var base_y = randi_range(30, 100)
	# 在屏幕的右边或左边，上边或下边
	var arr = [-1, 1]
	var is_right = arr[randi_range(0, 1)] > 0
	var is_bottom = arr[randi_range(0, 1)] > 0
	var x = (base_x + origin.x + size.x) if is_right else (base_x * -1)
	var y = (base_y + origin.y + size.y) if is_bottom else (base_y * -1)
